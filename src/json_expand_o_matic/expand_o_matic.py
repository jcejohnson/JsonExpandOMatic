...
class JsonExpandOMatic:
    ...
    def expand(self, data, root_element="root", preserve=True):
        ...
        return self._expand(
            path=self.path, key=root_element, data={root_element: data}, ref="."
        )
    ...
    def contract(self, root_element="root"):
        ...
        return self._contract(
            path=[self.path], data=self._slurp(self.path, f"{root_element}.json")
        )
        ...
        if isinstance(data, list):
            ...
            for k, v in data.items():
                if self._something_to_follow(k, v):
                    return self._contract(
                        path=path + [os.path.dirname(v)], data=self._slurp(*path, v)
                    )
                data[k] = self._contract(path=path, data=v)
